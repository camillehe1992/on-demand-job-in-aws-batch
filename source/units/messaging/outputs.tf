...
output "submit_batch_job_event_rule_arn" {
  ...
  value       = module.submit_batch_job_event_rule.event_rule_arn
}

output "capture_failed_batch_job_event_rule_arn" {
  description = "ARN of the EventBridge rule that captures only job-failed events"
  value       = module.capture_failed_batch_job_event_rule.event_rule_arn
}
